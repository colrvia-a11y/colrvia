  ...
  return (
    <div className="flex items-end gap-3" aria-label="Color swatches">
      {swatches.slice(0,5).map((s,i)=> (
        <button key={i} onClick={()=>copy(s.hex)} className={clsx('group relative h-14 w-14 rounded-xl border border-[var(--border)] shadow-soft focus:outline-none focus-visible:ring-2 focus-visible:ring-[var(--brand)] transition-colors', copied===s.hex && 'ring-2 ring-[var(--brand)]')} style={{background:s.hex}} aria-label={`Copy ${s.name||'swatch'} ${s.hex}`}>
