...
async function fetchStory(slug: string) {
  try {
    const res = await fetch(`${process.env.NEXT_PUBLIC_SITE_URL || ''}/api/share/by-slug/${slug}`, { cache: 'no-store' })
...
export default async function PublicSharePage({ params }: { params: { slug: string } }) {
  ...
  if ('notFound' in result) return <main className="mx-auto max-w-2xl p-6">Story not public or removed.</main>
  if ('error' in result) return <main className="mx-auto max-w-2xl p-6">Error loading story.</main>
  const { data } = result
  ...
  return (
    <main className="mx-auto max-w-3xl p-6 space-y-8">
      ...
      {story.narrative && <section className="prose max-w-none text-neutral-800"><p>{story.narrative}</p></section>}
      <footer className="pt-4 text-sm text-neutral-500"><a href="/designers" className="underline">Create your own at /designers</a></footer>
    </main>
  )
}
