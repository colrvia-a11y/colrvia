...
export default async function StoriesPage(){
    ...
    <main className="max-w-5xl mx-auto px-4 py-10 space-y-12">
      <header className="flex flex-col md:flex-row md:items-end md:justify-between gap-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground max-w-md">Drafts are unsaved working palettes. Add a title to save.</p>
        </div>
  ...
  <h2 className="text-sm font-medium tracking-wide uppercase text-muted-foreground">Drafts</h2>
        {drafts.length? <StoriesGrid stories={drafts} /> : <p className="text-xs text-muted-foreground">No drafts.</p>}
      </section>
      <section className="space-y-5">
  <h2 className="text-sm font-medium tracking-wide uppercase text-muted-foreground">Saved</h2>
        {saved.length? <StoriesGrid stories={saved} /> : <p className="text-xs text-muted-foreground">Nothing saved yet.</p>}
      </section>
      {!user && (
  <p className="text-xs text-muted-foreground">Sign in later to save – no account needed to explore.</p>
      )}
    </main>
  )
}
...
function StoriesGrid({ stories }: { stories:any[] }) {
  ...
  <p className="text-sm text-muted-foreground mb-6 max-w-xs">Start your first palette to see it appear here.</p>
