  ...
  return (
    <main className="max-w-4xl mx-auto px-4 py-10 space-y-10">
      <header>
        ...
        <p className="text-sm text-[var(--ink-subtle)] max-w-md">Inspiration & fresh palettes. (Placeholder content)</p>
      </header>
      <div className="grid gap-5 sm:grid-cols-2 md:grid-cols-3">
        {Array.from({ length:6 }).map((_,i)=> (
          <Link key={i} href="/start" className="rounded-xl border p-6 hover:shadow-sm transition-shadow">
            ...
            <p className="text-xs text-[var(--ink-subtle)]">Tap to start with this vibe.</p>
          </Link>
        ))}
      </div>
    </main>
