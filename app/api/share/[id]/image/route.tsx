...
// Public OG image endpoint for a story palette. Does not require auth so that social crawlers can render cards.
export async function GET(req: Request, { params }: { params:{ id:string } }) {
  const { searchParams } = new URL(req.url)
  const supabase = supabaseServer()
  const variant = searchParams.get('variant') // reserved for future variant-specific palette selection
  ...
  return new ImageResponse(
    <div style={{ fontFamily: 'system-ui, -apple-system, sans-serif', width:'100%', height:'100%', display:'flex', flexDirection:'column', background:'#F7F5EF', padding:64, justifyContent:'space-between' }}>
      <div style={{ fontSize:48, fontWeight:'600', lineHeight:1.05, maxWidth:900 }}>{title}</div>
