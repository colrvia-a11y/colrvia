...
export async function POST(req: Request){
  ...
  const title = (typeof body?.['title'] === 'string' && (body as any).title) || `${(body as any)?.style ?? (body as any)?.room ?? 'Design'} concept`
  const { data: created, error } = await supabase
    .from('stories')
    .insert({ user_id: user.id, title, status:'queued', input: body, idempotency_key })
    .select('id')
    .single()
  if(error || !created){
  ...
  try {
    await supabase.functions.invoke('render-worker', { body: { storyId: created.id, userId: user.id } })
  } catch {}
